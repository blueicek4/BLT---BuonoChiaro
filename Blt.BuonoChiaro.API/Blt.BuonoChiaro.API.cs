...
namespace Blt.BuonoChiaro.API
{
    ...
    public class BltBuonoChiaro : IPlugin
    {
                ...
                if (contrattoConto.Tool_Custom2 == "domanda")
                {
                    switch (contrattoConto.Tool_Custom1)
                    {
                        case "annullaBuonoElettronico":
                            ...
                            if (contrattoConto.Tool_RispostaFissa.HasValue && contrattoConto.Tool_RispostaFissa.Value.ToString().ToLower() == "si")
                            {
        ...
        public ContrattoConto PagamentoBuonoChiaroElettronico(ContrattoConto conto)
        {
            ...
            {
                DaPagare = DaPagare - conto.Pagamenti.Where(p => p.Tipo.Categoria == par.CategoriaPagamento).Sum(v => v.Importo);
            }
            if(DaPagare < 0)
            {
                conto.Tool_Domanda = String.Format("Impossibile inserire altri pagamenti con Buoni Pasto.\nTotale Conto: {0} - Totale Buoni inseriti: {1}", conto.TotaleDaPagare.Value, conto.TotaleDaPagare.Value - DaPagare);
            ...
            var posSetup = bceSetup.SelectSingleNode(String.Format("/ElencoPos/Pos[@ClientPC = '{0}']", conto.Tool_ClientPC));
            if (posSetup == null)
            {
                conto.Tool_Domanda = String.Format(@"Postazione : ""{0}"" non trovatA nel file di configurazione,\n controllare il file.", conto.Tool_ClientPC);
                conto.Tool_TipoDomanda = EnumToolTipoDomanda.Info;
                return conto;
    ...
    ,
                workstationID = posSetup.Attributes["workstationID"].Value
            };
            ...
            Helper.SetUltimoEvento(response.XML, response.workstationID);
            if (response.success)
            {
        ...
        public ContrattoConto StornaUltimaBuonoChiaroElettronico(ContrattoConto conto)
        {
            ...
            bceSetup.Load("FileConfigurazioneBuonoChiaroElettronico.xml");
            var posSetup = bceSetup.SelectSingleNode(String.Format("/ElencoPos/Pos[@ClientPC = '{0}']", conto.Tool_ClientPC));
            var LastTransaction = Helper.GetUltimoEvento(posSetup.Attributes["workstationID"].Value);
            if (LastTransaction == null)
            {
                conto.Tool_Domanda = "Impossibile Annullare l'ultima transazione.\nInformazioni mancanti sull'ultimo conto emesso";
                conto.Tool_TipoDomanda = EnumToolTipoDomanda.Info;
                return conto;
            }
            if (posSetup == null)
            ...
            GatewayPos.Gateway gw = new Gateway(Convert.ToInt32(posSetup.Attributes["portaLocale"].Value), posSetup.Attributes["indirizzoIpRemoto"].Value, Convert.ToInt32(posSetup.Attributes["portaRemota"].Value));
            gw.OnTransactionData += Gw_OnTransactionData;
            var response = gw.RefundLastTransaction(LastTransaction);
            if (response.success)
            {
                XmlDocument respXml = new XmlDocument();
                respXml.LoadXml(response.XML);
                ...
                var dataTransazione = DateTime.Now;
                ...
                conto.Tool_Domanda = String.Format("Annullamento Ultima transazione:\nIdTerminale: {0} - STAN Terminale : {1}\nData = {2}\nValore: {3} - Stato: {4}"
                    ...
                    , risultatoTransazione);
                conto.Tool_TipoDomanda = EnumToolTipoDomanda.Info;
                BuonoPasto bp = new BuonoPasto() { CodiceTransazione = LastTransaction.SelectSingleNode("CardServiceResponse/Tender/Authorisation/@ApprovalCode").Value };
                string message = string.Empty;
                if(par.RimuoviBuono(bp, out message))
                {
                    conto.Tool_Domanda += "\n\nBuono Pasto rimosso dal conto attuale.";
        ...
        public ContrattoConto ScriviPagamento(ContrattoConto conto, ParametriConto par)
        {
            ...
            if (pag.Count > 0 && !par.Codici.Select(c => c.CodiceABarre).All(c => pag.First().ListaBuoni.Any(b => b.Buono == c)))
            {
                ...
                {
                    var posSetup = bceSetup.SelectSingleNode(String.Format("/ElencoPos/vendor[@vendorID = '{0}']", c.Fornitore));
            ...
            else if (par.Codici.Count > 0)
            {
                ...
                {
                    var posSetup = bceSetup.SelectSingleNode(String.Format("/ElencoPos/vendor[@vendorID = '{0}']", codice.Fornitore));
